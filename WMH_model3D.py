...
def model3D(img=(83, 256, 256)):
    with tf.name_scope('WMH'):
        ...
        convStride = (1,1,1)
        poolStride = (2,2,2)
        seq.add(Conv3D(input_channels=1, num_filters=16, kernel_size=(3,3,3), stride=convStride, padding='SAME'))        
        ...
        seq.add(RELU())
        seq.add(Conv3D_Tranpose1(input_channels=32, num_filters=16, output_shape=layerSize1, kernel_size=(3,3,3), stride=convStride, padding='SAME'))
        seq.add(RELU())
        seq.add(Conv3D_Tranpose1(input_channels=16, num_filters=3, output_shape=img, kernel_size=(3,3,3), stride=(2,2,2), padding='SAME'))
        ##        
        seq.add(RELU())        
...
def model3D_2(img=(83, 256, 256)):
    with tf.name_scope('WMH_2Chan_Input'):
        ...
        poolStride = (2,2,2)
        kernelSize = (3,3,3)
        seq.add(Conv3D(input_channels=2, num_filters=8, kernel_size=(5,5,5), stride=convStride, padding='SAME'))        
        #seq.add(TFBatchNormalization(name='b1'))
        seq.add(MaxPool3D(poolsize=(2,2,2), stride=poolStride, padding='SAME'))
        layerSize1 = updateConvLayerSize(img,poolStride)
        #print("layer1: "+str(layerSize1))
        ...
        ##        
        seq.add(RELU())        
        seq.add(Conv3D(input_channels=3, num_filters=3, kernel_size=kernelSize, stride=convStride, padding='SAME'))        
        ##        
        seq.add(Softmax())
        #seq.add(Sigmoid())
    return seq
