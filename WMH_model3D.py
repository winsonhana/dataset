...
def model3D_2(img=(83, 256, 256)):
    with tf.name_scope('WMH_2Chan_Input'):
        ...
        #print("layer1: "+str(layerSize1))
        seq.add(RELU())
        seq.add(Conv3D(input_channels=10, num_filters=20, kernel_size=(5,5,5), stride=convStride, padding='SAME'))
        seq.add(TFBatchNormalization(name='b2'))
        #layerSize2 = updateConvLayerSize(layerSize1,convStride)
        #print("layer1: "+str(layerSize2))
        seq.add(RELU())
        seq.add(Conv3D_Tranpose1(input_channels=20, num_filters=10, output_shape=layerSize1, kernel_size=(5,5,5), stride=convStride, padding='SAME'))
        seq.add(RELU())
        seq.add(Conv3D_Tranpose1(input_channels=10, num_filters=3, output_shape=img, kernel_size=(5,5,5), stride=(2,2,2), padding='SAME'))
        ##        
        seq.add(RELU())        
        seq.add(Conv3D(input_channels=3, num_filters=3, kernel_size=(5,5,5), stride=convStride, padding='SAME'))        
        ##
        seq.add(Softmax())
        #seq.add(Sigmoid())
    return seq
